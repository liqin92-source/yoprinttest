...
<html lang="en">
    <head>
        ...
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/normalize/8.0.1/normalize.min.css" referrerpolicy="no-referrer">
        <style>
            body {
    ...
    <body>
        ...
        <script>
            ...
            const renderUploads = (uploads) => {
                ...
                if (!uploads.length) {
                    uploadsTableBody.innerHTML = '<tr><td colspan="4" style="text-align: center; padding: 24px;">No uploads yet.</td></tr>';
                    return;
                }

                uploads.forEach((upload) => {
                    const row = document.createElement('tr');
                    const progress = upload.total_rows ? `${upload.processed_rows || 0} / ${upload.total_rows}` : '-';
                    row.innerHTML = `
            ...
            uploadButton.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                        const message = payload?.message ?? payload?.errors?.file?.[0] ?? 'Upload failed';
                    ...
                    errorMessage.textContent = error.message || 'Unexpected error occurred.';
                    statusMessage.textContent = '';
